<template>
  <div class="wrapper">
    ...
    <div class="order-info">
      ...
      <p>{{deliveryaddress.contactName}}{{deliveryaddress.contactSex | sexFilter}} {{deliveryaddress.contactTel}}</p>
    </div>
    ...
    <ul class="order-detail">
      <li v-for="item in cartArr">
        <div class="order-detail-left">
          <img :src="require('../assets/'+item.food.foodImg+'.png')" />
...
export default {
  ...
  created() {
    ...
    this.deliveryaddress = this.$getLocalStorage(this.user.id);
  ...
  methods: {
    ...
    toPayment() {
      ...
      // 创建订单
      this.$axios.post('orders/saveOrders', this.$qs.stringify({
        userId: this.user.id,
        businessId: this.businessId,
        Total: this.totalPrice,
        daId: this.deliveryaddress.daId
      })).then(response => {
        let orderId = response.data.ordersId;
        if (orderId > 0) {
          this.$router.push({
            path: '/payment',
            query: {
